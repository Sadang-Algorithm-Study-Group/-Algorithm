...
function depthFirstSearch(map, name, index, count) {
    ...
    if (count > -1) {
        ...
        if (isAString(name) || prevIndexOfNotA < 0 || nextIndexOfNotA < 0) return count;
            ...
            let rightSum = depthFirstSearch(map, name, nextIndexOfNotA, count + (name.length - index + nextIndexOfNotA) % name.length);
            let leftSum = depthFirstSearch(map, name, prevIndexOfNotA, count + (name.length + index - prevIndexOfNotA) % name.length);
